...
async function daily(bot: Bot<BotContext>) {
	...
	const file = new InputFile(
		resolve(cwd(), "dist", "public", "JumaMuborak.jpg"),
	);
...
async function reminder(bot: Bot<BotContext>) {
	...
	for (const regionId of activeRegionIds) {
		...
		schedule.scheduleJob(
			{ hour: +fajr[0], minute: +fajr[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
				...
				for (const user of users) {
					await bot.api
						.sendMessage(
							user.userId,
							user.fasting ? t(($) => $.closeFast) : t(($) => $.fajrTime),
						)
						.catch(async (err) => {
		...
		schedule.scheduleJob(
			{ hour: +sunrise[0], minute: +sunrise[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
		...
		schedule.scheduleJob(
			{ hour: +dhuhr[0], minute: +dhuhr[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
		...
		schedule.scheduleJob(
			{ hour: +asr[0], minute: +asr[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
		...
		schedule.scheduleJob(
			{ hour: +maghrib[0], minute: +maghrib[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
				...
				for (const user of users) {
					await bot.api
						.sendMessage(
							user.userId,
							user.fasting ? t(($) => $.breakFast) : t(($) => $.maghribTime),
						)
						.catch(async (err) => {
							await handleUserSendMessageError(err, user);
						});
		...
		schedule.scheduleJob(
			{ hour: +isha[0], minute: +isha[1], tz: "Asia/Tashkent" },
			async () => {
				const users = await Model.User.find<IUser>({
					regionId,
					deletedAt: null,
