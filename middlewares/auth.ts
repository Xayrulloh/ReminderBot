...
export async function userAuthMiddleware(ctx: BotContext, next: NextFunction) {
	...
	if (!user) {
		const isStartSceneActive = ctx.session.scenes?.stack?.some(
			(stack) => stack.scene === "Start",
		);
...
export async function groupAuthMiddleware(ctx: BotContext, next: NextFunction) {
	...
	if (!group || !group.status) {
		if (
			ctx.session.scenes?.stack?.some((stack) => stack.scene === "GroupStart")
		) {
			return next();
		}
	...
	const isInteractingWithBot =
		...
		(ctx.me?.username &&
			(ctx.message?.text?.includes(`@${ctx.me.username}`) ||
				ctx.message?.caption?.includes(`@${ctx.me.username}`)));
