...
newScene.do(async (ctx) => {
  ...
  ctx.reply("Yangi joyni belgilang", {
...
newScene.wait().on("message:text", async (ctx) => {
  if (
    ...
    if (ctx.message.text == res.location) {
      ctx.reply(
        `Joylashuvingiz o'z holicha qoldi. Sababi oldin ham sizning joylashuvingiz ${res.location} bo'lgan ekan`,
      ...
      ctx.reply("Joylashuvingiz o'zgartirildi", {
        reply_markup: { remove_keyboard: true },
      });
      ctx.scene.exit();
    }
