...
scene.step(async (ctx) => {
    ...
    if ([1151533771, 900604435, 962526857].includes(ctx.user.userId)) {
        const usersInfo = users.reduce(
            (userObj, user) => {
                if (user.status === false && !user.deletedAt) {
                    ++userObj.blockedUsers;
                }
        ...
        shareMessage +=
            `.\n\n Blocked users:  ${usersInfo.blockedUsers}\n Deleted account users: ${usersInfo.deletedUsers}\n Pure users:   ${
                users.length - (usersInfo.blockedUsers + usersInfo.deletedUsers)
            }`;
    }
